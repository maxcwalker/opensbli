...
gridx1=parse_expr("Eq(DataObject(x1), Ly*sinh(b*block.deltas[1]*block.grid_indexes[1]/Ly)/sinh(b)-(sinh(b*block.deltas[1]*block.grid_indexes[1]/Ly)/sinh(b)-1.0)*0.5*tramp*(block.grid_indexes[0]*block.grid_indexes[0]-x0)*(1.0+tanh((block.deltas[0]*block.grid_indexes[0]-x0)/(Rcorn+Rcurv*Lx*block.deltas[1]*block.grid_indexes[1]))))", local_dict=local_dict)
