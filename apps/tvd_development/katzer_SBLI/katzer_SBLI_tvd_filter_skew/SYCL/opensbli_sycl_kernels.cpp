...
cl::sycl::buffer<double,1> *mu_p=nullptr;
extern double mu;
...
void ops_decl_const_char(OPS_instance *instance, int dim, char const * type, int size, char * dat, char const * name ) {
  ...
  if (!strcmp(name,"mu")) {
    if (mu_p == nullptr) mu_p = new cl::sycl::buffer<double,1>(cl::sycl::range<1>(dim));
    auto accessor = (*mu_p).get_access<cl::sycl::access::mode::write>();
    for ( int d=0; d<dim; d++ ){
      accessor[d] = ((double*)dat)[d];
    }
  }
