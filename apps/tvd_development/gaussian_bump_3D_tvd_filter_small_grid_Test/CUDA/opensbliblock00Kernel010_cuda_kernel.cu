...
void opensbliblock00Kernel010_gpu(const ACC<double> &u1_B0,
  ACC<double> &wk1_B0,
  const int *idx)
{
   if (idx[0] == 0){
...
void ops_par_loop_opensbliblock00Kernel010(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  create_kerneldesc_and_enque(name, args, 3, 21, dim, 1, range, block, ops_par_loop_opensbliblock00Kernel010_execute);
}
#endif
