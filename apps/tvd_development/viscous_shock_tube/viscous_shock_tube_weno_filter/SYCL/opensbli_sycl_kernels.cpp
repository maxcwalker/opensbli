...
cl::sycl::buffer<double,1> *Ducros_check_p=nullptr;
...
void ops_decl_const_char(OPS_instance *instance, int dim, char const * type, int size, char * dat, char const * name ) {
  ...
  if (!strcmp(name,"Ducros_check")) {
    if (Ducros_check_p == nullptr) Ducros_check_p = new cl::sycl::buffer<double,1>(cl::sycl::range<1>(dim));
    auto accessor = (*Ducros_check_p).get_access<cl::sycl::access::mode::write>();
    for ( int d=0; d<dim; d++ ){
      accessor[d] = ((double*)dat)[d];
    }
  }
