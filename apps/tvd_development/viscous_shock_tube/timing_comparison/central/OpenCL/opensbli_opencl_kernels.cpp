...
void ops_decl_const_char(OPS_instance *instance, int dim, char const * type, int typeSize, char * dat, char const * name ) {
  ...
  if (instance->opencl_instance->OPS_opencl_core.constant == NULL) {
    instance->opencl_instance->OPS_opencl_core.constant = (cl_mem*) malloc((23)*sizeof(cl_mem));
    for ( int i=0; i<23; i++ ){
  ...
  void buildOpenCLKernels(OPS_instance *instance) {
    ...
    if(!isbuilt) {
      ...
      instance->opencl_instance->OPS_opencl_core.n_kernels = 18;
      instance->opencl_instance->OPS_opencl_core.kernel = (cl_kernel*) malloc(18*sizeof(cl_kernel));
    }
    isbuilt = true;
  }
