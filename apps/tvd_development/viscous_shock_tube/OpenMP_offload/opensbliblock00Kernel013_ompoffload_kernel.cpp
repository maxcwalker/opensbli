...
void ops_par_loop_opensbliblock00Kernel013_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start1; n_y<end1; n_y++ ){
    for ( int n_x=start0; n_x<end0; n_x++ ){
      ...
      ACC<double> T_B0(xdim2_opensbliblock00Kernel013, T_B0_p + n_x*1 + n_y * xdim2_opensbliblock00Kernel013*1);
...
void ops_par_loop_opensbliblock00Kernel013(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  create_kerneldesc_and_enque(name, args, 3, 8, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel013_execute);
}
#endif
