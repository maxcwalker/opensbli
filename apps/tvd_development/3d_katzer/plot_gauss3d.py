...
CS=ax.contourf(x[0,:,:],y[0,:,:],u[0,:,:],levels=50, cmap=cm.jet)
...
try:
    U = axins1.contourf(x[:,y_idx,:], z[:,y_idx,:], u[:,y_idx,:],levels= list(np.linspace(np.min(u[:,y_idx,:]), 0, 5))+list(np.linspace(0.001,np.max(u[:,y_idx,100:200]),30)), cmap=cm.jet)
    recirc=axins1.contour(x[:,y_idx,:], z[:,y_idx,:], u[:,y_idx,:],levels=[0],colors='black')
except:
    U = axins1.contourf(x[:,y_idx,:], z[:,y_idx,:], u[:,y_idx,:],levels= np.linspace(np.min(u[:,y_idx,:]), np.max(u[:,y_idx]), 25), cmap=cm.jet)
