...
void ops_par_loop_opensbliblock00Kernel043_execute(ops_kernel_descriptor *desc) {
  ...
  double * __restrict__ wdotO2_B0_p = (double *)(args[0].data_d + base0);
  ...
  for ( int n_x=start0; n_x<end0; n_x++ ){
    ACC<double> wdotO2_B0(wdotO2_B0_p + n_x*1);
...
void ops_par_loop_opensbliblock00Kernel043(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0) {
  ...
  create_kerneldesc_and_enque(name, args, 1, 10, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel043_execute);
}
#endif
