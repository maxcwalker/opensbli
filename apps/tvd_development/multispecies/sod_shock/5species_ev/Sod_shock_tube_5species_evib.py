...
input_dict = {
    ...
    "kappa_TVD"            : "0.7",
    "MN"                   : "0.5", #mole fractions normalised by MN2 (28.0)    
    "MN2"                  : "1.0",
    "MO"                  : '4.0/7.0',
    "MO2"                  : "8.0/7.0",
    "MNO"                  : "15.0/14.0",
    ...
    "Rhat"                 : "1.0",#8314.3*300.0/(340.0*340.0)",#8314.3*600.0/340.0*340.0",
    "kappa"                : "0.0",   
}
...
temperature = "Eq(T, (rhoE -rhoev - dhf - (rhoO+rhoO2+rhoN+rhoN2+rhoNO)*(1./2.)*(KD(_i,_j)*u_i*u_j)-rhoev)/(Rhat*(3.0/2.0*(rhoO/MO+rhoN/MN)+5.0/2.0*(rhoO2/MO2+rhoN2/MN2+rhoNO/MNO))) )"
# vibrational terms
...
# Initial conditions
cNl, cNr = 0.6984386275169115, 0.0
cN2l, cN2r = 0.06607169490642878, 0.7653
cOl, cOr = 0.2342744649144739, 0.0
cO2l, cO2r = 2.663297558683734e-05, 0.2347
cNOl, cNOr = 0.001188579686599012, 6.178621075754453e-19
...
p = "Eq(GridVariable(p0), Piecewise((1.0, DataObject(x0) < 0.5), (0.051282, True)))"
T = "Eq(GridVariable(T0), Piecewise((1.0, DataObject(x0) < 0.5), (0.033333, True)))"
r = "Eq(GridVariable(r), p0*(1/Rhat)/(T0*(cO2/MO2 + cO/MO + cN2/MN2 + cNO/MNO + cN/MN)))"

cN = "Eq(GridVariable(cN), Piecewise((0.6984386275169115, DataObject(x0) < 0.5), (0.0, True)))"
cN2 = "Eq(GridVariable(cN2), Piecewise((0.06607169490642878, DataObject(x0) < 0.5), (0.7653, True)))"
cO = "Eq(GridVariable(cO), Piecewise((0.2342744649144739, DataObject(x0) < 0.5), (0.0, True)))"
cO2 = "Eq(GridVariable(cO2), Piecewise((2.663297558683734e-05, DataObject(x0) < 0.5), (0.2347, True)))"
cNO = "Eq(GridVariable(cNO), Piecewise((0.001188579686599012, DataObject(x0) < 0.5), (6.178621075754453e-19, True)))"
...
eqns = [x0, u0, p, T, cN,cN2, cO, cO2, cNO, r, rhoN, rhoN2, rhoO, rhoO2, rhoNO, rhou0, evN2, evO2, evNO, rhoev,  rhoE]
local_dict = {"block": block, "GridVariable": GridVariable, "DataObject": DataObject}
initial_equations = [parse_expr(eq, local_dict=local_dict) for eq in eqns]
initial = GridBasedInitialisation()
initial.add_equations(initial_equations)

r, u, p,T, cN, cN2, cO, cO2, cNO = symbols('r u0 p0 T0 cN cN2 cO cO2 cNO', **{'cls':GridVariable})
# Left boundary condition
left_eqns = [ OpenSBLIEq(u, 0.0), OpenSBLIEq(p, pl), OpenSBLIEq(T,Tl), OpenSBLIEq(cN, cNl),  OpenSBLIEq(cN2, cN2l),  OpenSBLIEq(cO, cOl),  OpenSBLIEq(cO2, cO2l),  OpenSBLIEq(cNO, cNOl),\
OpenSBLIEq(r, pl*(1/Rhat)/(Tl*(cO2l/MO2 + cOl/MO + cN2l/MN2 + cNOl/MNO + cNl/MN)))] + [parse_expr(eq, local_dict=local_dict) for eq in [rhoN, rhoN2, rhoO, rhoO2, rhoNO,  rhou0, evN2, evO2, evNO, rhoev, rhoE]] #*(cOl/MO+cO2l/MO2+cNl/MN+cN2l/MN2+cNOl/MNO)
# Right boundary condition
right_eqns = [OpenSBLIEq(u, 0.0),  OpenSBLIEq(p, pr), OpenSBLIEq(T,Tr), OpenSBLIEq(cN, cNr),  OpenSBLIEq(cN2, cN2r),  OpenSBLIEq(cO, cOr),  OpenSBLIEq(cO2, cO2r),  OpenSBLIEq(cNO, cNOr),\
OpenSBLIEq(r, pr*(1/Rhat)/(Tr*(cO2r/MO2 + cOr/MO + cN2r/MN2 + cNOr/MNO + cNr/MN)))] + [parse_expr(eq, local_dict=local_dict) for eq in [rhoN, rhoN2, rhoO, rhoO2, rhoNO, rhou0, evN2, evO2, evNO, rhoev, rhoE]] #*(cOr/MO+cO2r/MO2+cNr/MN+cN2r/MN2+cNOr/MNO)
