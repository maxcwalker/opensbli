...
substitutions = [heat_flux, evibration, molesum, hformation]
...
constituent_eqns = [velocity, pressure, consts] 
...
eqns = [x0, u0, r, p, T, cN,cN2, rhoN, rhoN2, rhou0, rhoE]
...
OpenSBLIEq(T, pl*(1/Rhat)/(rl))] + [parse_expr(eq, local_dict=local_dict) for eq in [rhoN, rhoN2, rhou0, rhoE]]
# Right boundary condition
right_eqns = [OpenSBLIEq(r, rr), OpenSBLIEq(u, 0.0),  OpenSBLIEq(p, pr), OpenSBLIEq(cN2, cN2r),  OpenSBLIEq(cN, cNr),\
OpenSBLIEq(T, pr*(1/Rhat)/(rr))] + [parse_expr(eq, local_dict=local_dict) for eq in [rhoN, rhoN2, rhou0, rhoE]]
