...
void ops_par_loop_opensbliblock00Kernel034_execute(ops_kernel_descriptor *desc) {
 ...
 arg30, arg31, arg32, arg33};
  ...
  double* ptauN2_B0_p = (double*)args[7].data_d;

  int base8 = args[8].dat->base_offset/sizeof(double);
  double* ptauNO_B0_p = (double*)args[8].data_d;

  int base9 = args[9].dat->base_offset/sizeof(double);
  double* ptauO2_B0_p = (double*)args[9].data_d;

  int base10 = args[10].dat->base_offset/sizeof(double);
  double* rhoE_B0_p = (double*)args[10].data_d;

  int base11 = args[11].dat->base_offset/sizeof(double);
  double* rhoN2_B0_p = (double*)args[11].data_d;
  ...
  double* rhoN_B0_p = (double*)args[13].data_d;

  int base14 = args[14].dat->base_offset/sizeof(double);
  double* rhoO2_B0_p = (double*)args[14].data_d;

  int base15 = args[15].dat->base_offset/sizeof(double);
  double* rhoO_B0_p = (double*)args[15].data_d;

  int base16 = args[16].dat->base_offset/sizeof(double);
  double* rhoev_B0_p = (double*)args[16].data_d;
  ...
  if ((end[0]-start[0])>0) {
    block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
      ...
      cgh.parallel_for<class opensbliblock00Kernel034_kernel>(cl::sycl::nd_range<1>(cl::sycl::range<1>(
      ...
      , [=](cl::sycl::nd_item<1> item
        ...
        const ACC<double> evN2_B0(&evN2_B0_p[0] + base0 + n_x*1);
        const ACC<double> evNO_B0(&evNO_B0_p[0] + base1 + n_x*1);
        const ACC<double> evO2_B0(&evO2_B0_p[0] + base2 + n_x*1);
        const ACC<double> eveqN2_B0(&eveqN2_B0_p[0] + base3 + n_x*1);
        const ACC<double> eveqNO_B0(&eveqNO_B0_p[0] + base4 + n_x*1);
        const ACC<double> eveqO2_B0(&eveqO2_B0_p[0] + base5 + n_x*1);
        const ACC<double> p_B0(&p_B0_p[0] + base6 + n_x*1);
        const ACC<double> ptauN2_B0(&ptauN2_B0_p[0] + base7 + n_x*1);
        const ACC<double> ptauNO_B0(&ptauNO_B0_p[0] + base8 + n_x*1);
        const ACC<double> ptauO2_B0(&ptauO2_B0_p[0] + base9 + n_x*1);
        const ACC<double> rhoE_B0(&rhoE_B0_p[0] + base10 + n_x*1);
        const ACC<double> rhoN2_B0(&rhoN2_B0_p[0] + base11 + n_x*1);
        const ACC<double> rhoNO_B0(&rhoNO_B0_p[0] + base12 + n_x*1);
        const ACC<double> rhoN_B0(&rhoN_B0_p[0] + base13 + n_x*1);
        const ACC<double> rhoO2_B0(&rhoO2_B0_p[0] + base14 + n_x*1);
        const ACC<double> rhoO_B0(&rhoO_B0_p[0] + base15 + n_x*1);
        const ACC<double> rhoev_B0(&rhoev_B0_p[0] + base16 + n_x*1);
        const ACC<double> rhou0_B0(&rhou0_B0_p[0] + base17 + n_x*1);
        const ACC<double> u0_B0(&u0_B0_p[0] + base18 + n_x*1);
        const ACC<double> wdotN2_B0(&wdotN2_B0_p[0] + base19 + n_x*1);
        const ACC<double> wdotNO_B0(&wdotNO_B0_p[0] + base20 + n_x*1);
        const ACC<double> wdotN_B0(&wdotN_B0_p[0] + base21 + n_x*1);
        const ACC<double> wdotO2_B0(&wdotO2_B0_p[0] + base22 + n_x*1);
        const ACC<double> wdotO_B0(&wdotO_B0_p[0] + base23 + n_x*1);
        const ACC<double> wk0_B0(&wk0_B0_p[0] + base24 + n_x*1);
   ...
   if (idx[0] == 0){
    ...
    Residual6_B0(0) = -(1.0/2.0)*d1_rhoevu0_dx + evN2_B0(0)*wdotN2_B0(0) + evNO_B0(0)*wdotNO_B0(0) +
      ...
      9.86923266716013e-5*(invMN2_sycl[0]*rhoN2_B0(0)/ptauN2_B0(0) + invMNO_sycl[0]*rhoNO_B0(0)/ptauNO_B0(0) +
