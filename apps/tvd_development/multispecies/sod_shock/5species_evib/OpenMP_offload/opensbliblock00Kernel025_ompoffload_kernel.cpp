...
void ops_par_loop_opensbliblock00Kernel025_execute(ops_kernel_descriptor *desc) {
  ...
  double * __restrict__ evO2_B0_p = (double *)(args[1].data_d + base1);
  ...
  for ( int n_x=start0; n_x<end0; n_x++ ){
    ...
    ACC<double> evO2_B0(evO2_B0_p + n_x*1);
...
void ops_par_loop_opensbliblock00Kernel025(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  create_kerneldesc_and_enque(name, args, 2, 13, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel025_execute);
}
#endif
