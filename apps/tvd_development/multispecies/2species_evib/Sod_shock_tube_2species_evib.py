...
hformation = "Eq(dhf, 4.1868e6*(dhN*rhoN/MN))"
...
constituent_eqns = [velocity, pressure, temperature, timefactorN2, evequilN2, evN2, consts] 
...
left_eqns = [OpenSBLIEq(r, 1.0), OpenSBLIEq(u, 0.0), OpenSBLIEq(p, pl), OpenSBLIEq(cN2, cN2l),  OpenSBLIEq(cN, cNl),\
OpenSBLIEq(T, pl*(1/Rhat)/(rl))] + [parse_expr(eq, local_dict=local_dict) for eq in [rhoN, rhoN2, rhou0, evN2, rhoev, rhoE]]
# Right boundary condition
right_eqns = [OpenSBLIEq(r, 0.2), OpenSBLIEq(u, 0.0),  OpenSBLIEq(p, pr), OpenSBLIEq(cN2, cN2r),  OpenSBLIEq(cN, cNr),\
...
concentration_data = ['cN2', 'cN']
physical_values    = [  str(cN2l), str(cNl)]
