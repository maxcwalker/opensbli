...
void ops_par_loop_opensbliblock00Kernel030_execute(ops_kernel_descriptor *desc) {
  ...
  double * __restrict__ Residual1_B0_p = (double *)(args[0].data_d + base0);

  int base1 = args[1].dat->base_offset;
  double * __restrict__ Residual2_B0_p = (double *)(args[1].data_d + base1);

  int base2 = args[2].dat->base_offset;
  double * __restrict__ Residual0_B0_p = (double *)(args[2].data_d + base2);

  int base3 = args[3].dat->base_offset;
  double * __restrict__ tempRK_rho_B0_p = (double *)(args[3].data_d + base3);

  int base4 = args[4].dat->base_offset;
  double * __restrict__ tempRK_rhoE_B0_p = (double *)(args[4].data_d + base4);
  ...
  double * __restrict__ tempRK_rhou0_B0_p = (double *)(args[7].data_d + base7);
  ...
  double *arg9h = (double *)args[9].data;
  double *arg10h = (double *)args[10].data;

  int consts_bytes = 0;
  consts_bytes += ROUND_UP(args[9].dim*sizeof(double));
  consts_bytes += ROUND_UP(args[10].dim*sizeof(double));

  reallocConstArrays(block->instance,consts_bytes);
  consts_bytes = 0;

  args[9].data = block->instance->OPS_consts_h + consts_bytes;
  args[9].data_d = block->instance->OPS_consts_d + consts_bytes;
  for ( int d=0; d<args[9].dim; d++ ){
    ((double *)args[9].data)[d] = arg9h[d];
  }
  consts_bytes += ROUND_UP(args[9].dim*sizeof(double));

  args[10].data = block->instance->OPS_consts_h + consts_bytes;
  args[10].data_d = block->instance->OPS_consts_d + consts_bytes;
  for ( int d=0; d<args[10].dim; d++ ){
    ((double *)args[10].data)[d] = arg10h[d];
  }
  consts_bytes += ROUND_UP(args[10].dim*sizeof(double));

  mvConstArraysToDevice(block->instance,consts_bytes);

  double * __restrict__ rkB = (double *)args[9].data_d;
  double * __restrict__ rkA = (double *)args[10].data_d;
...
void ops_par_loop_opensbliblock00Kernel030(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg8, ops_arg arg9, ops_arg arg10) {
  ...
  create_kerneldesc_and_enque(name, args, 11, 20, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel030_execute);
}
#endif
