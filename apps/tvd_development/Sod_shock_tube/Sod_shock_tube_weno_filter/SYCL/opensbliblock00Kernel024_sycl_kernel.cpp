...
void ops_par_loop_opensbliblock00Kernel024_execute(ops_kernel_descriptor *desc) {
 ...
 arg15, arg16, arg17, arg18};
  ...
  double* tau00_B0_p = (double*)args[3].data_d;
  ...
  double* wk10_B0_p = (double*)args[5].data_d;

  int base6 = args[6].dat->base_offset/sizeof(double);
  double* wk11_B0_p = (double*)args[6].data_d;

  int base7 = args[7].dat->base_offset/sizeof(double);
  double* wk12_B0_p = (double*)args[7].data_d;

  int base8 = args[8].dat->base_offset/sizeof(double);
  double* wk13_B0_p = (double*)args[8].data_d;
  ...
  if ((end[0]-start[0])>0) {
    block->instance->sycl_instance->queue->submit([&](cl::sycl::handler &cgh) {
      ...
      , [=](cl::sycl::nd_item<1> item
        ...
        const ACC<double> rho_B0(&rho_B0_p[0] + base1 + n_x*1);
        const ACC<double> rhou0_B0(&rhou0_B0_p[0] + base2 + n_x*1);
        const ACC<double> tau00_B0(&tau00_B0_p[0] + base3 + n_x*1);
        const ACC<double> u0_B0(&u0_B0_p[0] + base4 + n_x*1);
        const ACC<double> wk10_B0(&wk10_B0_p[0] + base5 + n_x*1);
        const ACC<double> wk11_B0(&wk11_B0_p[0] + base6 + n_x*1);
        const ACC<double> wk12_B0(&wk12_B0_p[0] + base7 + n_x*1);
        const ACC<double> wk13_B0(&wk13_B0_p[0] + base8 + n_x*1);
        const ACC<double> wk3_B0(&wk3_B0_p[0] + base9 + n_x*1);
        const ACC<double> wk4_B0(&wk4_B0_p[0] + base10 + n_x*1);
        const ACC<double> wk5_B0(&wk5_B0_p[0] + base11 + n_x*1);
        const ACC<double> wk6_B0(&wk6_B0_p[0] + base12 + n_x*1);
        const ACC<double> wk7_B0(&wk7_B0_p[0] + base13 + n_x*1);
        const ACC<double> wk8_B0(&wk8_B0_p[0] + base14 + n_x*1);
        const ACC<double> wk9_B0(&wk9_B0_p[0] + base15 + n_x*1);
        ACC<double> Residual0_B0(&Residual0_B0_p[0] + base16 + n_x*1);
        ACC<double> Residual1_B0(&Residual1_B0_p[0] + base17 + n_x*1);
        ACC<double> Residual2_B0(&Residual2_B0_p[0] + base18 + n_x*1);
        //USER CODE
        if (n_x < end_0) {
