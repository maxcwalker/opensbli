...
void ops_par_loop_opensbliblock00Kernel005_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start1; n_y<end1; n_y++ ){
    for ( int n_x=start0; n_x<end0; n_x++ ){
      ...
      const ACC<double> rhou1_B0(xdim1_opensbliblock00Kernel005, rhou1_B0_p + n_x*1 + n_y * xdim1_opensbliblock00Kernel005*1);
...
void ops_par_loop_opensbliblock00Kernel005(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  create_kerneldesc_and_enque(name, args, 3, 23, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel005_execute);
}
#endif
