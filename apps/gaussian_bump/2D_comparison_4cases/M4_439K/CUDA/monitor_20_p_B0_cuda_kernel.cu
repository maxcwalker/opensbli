...
//
__constant__ int dims_monitor_20_p_B0 [2][1];
static int dims_monitor_20_p_B0_h [2][1] = {{0}};
...
int size1 ){
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_monitor_20_p_B0[0][0];

  if (idx_x < size0 && idx_y < size1) {
    const ACC<double> argp0(dims_monitor_20_p_B0[0][0], arg0);
    monitor_20_p_B0_gpu(
     argp0, arg1_l);
  }
  for (int d=0; d<1; d++)
...
void ops_par_loop_monitor_20_p_B0_execute(ops_kernel_descriptor *desc) {
  ...
  if (xdim0 != dims_monitor_20_p_B0_h[0][0]) {
...
void ops_par_loop_monitor_20_p_B0(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  create_kerneldesc_and_enque(name, args, 2, 73, dim, 1, range, block, ops_par_loop_monitor_20_p_B0_execute);
}
#endif
