...
void ops_par_loop_monitor_15_p_B0_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start[1]; n_y<end[1]; n_y++ ){
    ...
    for ( int n_x=start[0]; n_x<end[0]; n_x++ ){
      const ACC<double> p_B0(xdim0_monitor_15_p_B0, p_B0_p + n_x*1 + n_y * xdim0_monitor_15_p_B0*1);
...
void ops_par_loop_monitor_15_p_B0(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  create_kerneldesc_and_enque(name, args, 2, 45, dim, 0, range, block, ops_par_loop_monitor_15_p_B0_execute);
}
#endif
