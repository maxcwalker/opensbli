...
void ops_par_loop_opensbliblock00Kernel041_execute(ops_kernel_descriptor *desc) {
  ...
  for ( int n_y=start1; n_y<end1; n_y++ ){
    for ( int n_x=start0; n_x<end0; n_x++ ){
      ...
      const ACC<double> wk14_B0(xdim0_opensbliblock00Kernel041, wk14_B0_p + n_x*1 + n_y * xdim0_opensbliblock00Kernel041*1);
...
void ops_par_loop_opensbliblock00Kernel041(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  create_kerneldesc_and_enque(name, args, 3, 50, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel041_execute);
}
#endif
