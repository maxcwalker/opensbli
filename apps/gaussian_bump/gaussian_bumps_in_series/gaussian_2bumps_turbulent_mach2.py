...
gridx1 = parse_expr("Eq(DataObject(x1), H/20*exp(-((block.deltas[0]*block.grid_indexes[0]-L/3)/a)**2) + H/20*exp(-((block.deltas[0]*block.grid_indexes[0]-2*L/3)/a)**2)+ (H - H/20*exp(-((block.deltas[0]*block.grid_indexes[0]-L/3)/a)**2) - H/20*exp(-((block.deltas[0]*block.grid_indexes[0]-2*L/3)/a)**2))*sinh(b*block.deltas[1]*block.grid_indexes[1]/H)/sinh(b))", local_dict=local_dict)
