...
class Utilities(object):
    ...
    def read_full_dataset(self, group, dataset):
        d_m = group["%s" % (dataset)].attrs['d_m']
        size = group["%s" % (dataset)].shape
        read_data = group["%s" % (dataset)][:,:,:]
        return read_data

    def extract_full_data(self, fname, variable):
        """ Extracts the data from the HDF5 file. Data is stored in the self.data dictionary."""
        f, group1 = self.read_file(fname)
        output = self.read_full_dataset(group1, variable + '_B0')
        f.close()
        return output
...
class ReGrid(Utilities):
    ...
    def generate_new_y_values(self,xs):
        ...
        y = self.H/20*np.exp(-((xs/self.nx_new-self.L/2)/self.a)**2) + (self.H - self.H/20*np.exp(-((xs/self.nx_new-self.L/2)/self.a)**2))*np.sinh(self.stretch_factor*j/self.ny_new/self.H)/np.sinh(self.stretch_factor)
        return y
    
    def generate_new_y_values_full(self,xs):
        """ Generates the y coordinate values using the hyperbolic tan stretching function."""
        
        j = np.array([i for i in range(self.ny_new+self.n_halos*2)])

        # y = 0.5*self.Ly*(1.0-((np.tanh(self.stretch_factor*(1.0-2.0*(j/(self.ny_new-1.0)))))/(np.tanh(self.stretch_factor))))-1.0

        y = self.H/20*np.exp(-((xs/self.nx_new+self.n_halos*2-self.L/2)/self.a)**2) + (self.H - self.H/20*np.exp(-((xs/self.nx_new+self.n_halos*2-self.L/2)/self.a)**2))*np.sinh(self.stretch_factor*j/self.ny_new+self.n_halos*2/self.H)/np.sinh(self.stretch_factor)
        return y
    ...
    def zero_out_domain(self,dset):
        nh = self.n_halos
        zero_domain_size = np.zeros((self.nz_new, self.ny_new, self.nx_new))
        dset[nh:-nh, nh:-nh, nh:-nh] = zero_domain_size
        del zero_domain_size
        gc.collect()
        return dset
    ...
    def main(self):
        ...
        y_old_full = self.read_full_dataset(group1, 'x1_B0')
        nz_old, ny_old, nx_old = y_old.shape
        x_old, z_old = self.create_x_z_coordinates(y_old)
        x_old_full, z_old_full = self.create_x_z_coordinates(y_old_full)
        # Free some memory
        f.close()
        del group1
        gc.collect()
        ...
        x_new_full, z_new_full = np.linspace(x_old_full[0], x_old_full[-1], self.nx_new+self.n_halos*2), np.linspace(z_old_full[0], z_old_full[-1], self.nz_new+self.n_halos*2)
        # y coordinates for the new mesh
        k = np.array([i for i in range(self.nx_new)])
        ...
        for var_no, var in enumerate(variables):
            ...
            for k in range(nz_old):
                for i in range(nx_old):
                    y_new = self.generate_new_y_values(i)
            ...
            original_data_full = self.extract_full_data(input_file,var)
            y_extend_full = np.zeros((nz_old+self.n_halos*2, self.ny_new+self.n_halos*2, nx_old+self.n_halos*2))
            # Extend in y and interpolate over the grid
            print("Performing the interpolation in the y direction.")
            for k in range(nz_old+self.n_halos*2):
                for i in range(nx_old+self.n_halos*2):
                    y_new_full = self.generate_new_y_values_full(i)
                    data_line = original_data_full[k, :, i]
                    f = interpolate.splrep(y_old_full, data_line, s=0)
                    interpolated_y_data = interpolate.splev(y_new_full, f, der=0)
                    y_extend_full[k, :, i] = interpolated_y_data
            del original_data_full
            gc.collect()
            print("Performing the interpolation in the x direction.")
            x_extend_full = np.zeros((nz_old+self.n_halos*2, self.ny_new+self.n_halos*2, self.nx_new+self.n_halos*2))
            for k in range(nz_old+self.n_halos*2):
                for j in range(self.ny_new+self.n_halos*2):
                    data_line = y_extend_full[k, j, :]
                    f = interpolate.splrep(x_old_full, data_line, s=0)
                    interpolated_x_data = interpolate.splev(x_new_full, f, der=0)
                    x_extend_full[k, j, :] = interpolated_x_data
            del y_extend_full
            gc.collect()
            print("Performing the interpolation in the z direction.")
            z_extend_full = np.zeros((self.nz_new+self.n_halos*2, self.ny_new+self.n_halos*2, self.nx_new+self.n_halos*2))
            for i in range(self.nx_new+self.n_halos*2):
                for j in range(self.ny_new+self.n_halos*2):
                    data_line = x_extend_full[:, j, i]
                    f = interpolate.splrep(z_old_full, data_line, s=0)
                    interpolated_z_data = interpolate.splev(z_new_full, f, der=0)
                    z_extend_full[:, j, i] = interpolated_z_data
            # Make some plots
            # self.plot_check(y_old, original_data, y_new, z_extend, var)
            del x_extend_full
            gc.collect()

            z_extend_full = self.zero_out_domain(z_extend_full)
            zeros = np.where(z_extend_full == 0.0)
            z_extend_full[zeros] = z_extend[zeros] 
