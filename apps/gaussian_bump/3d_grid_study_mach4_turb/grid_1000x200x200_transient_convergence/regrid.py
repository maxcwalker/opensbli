...
class ReGrid(Utilities):
    def __init__(self, new_grid):
        ...
        self.ndim = 3
        self.Lx, self.Ly, self.Lz = 400.0, 115.0, 20.0
        self.a = 20.0
        self.b = 5.0
        self.n_halos = 5
    ...
    def generate_new_y_values(self,xs):
        ...
        y = self.Ly/20*np.exp(-((xs/(self.nx_new-1)*self.Lx-self.Lx/2)/self.a)**2) + (self.Ly - self.Ly/20*np.exp(-((xs/(self.nx_new-1)*self.Lx-self.Lx/2)/self.a)**2))*np.sinh(self.b*j/(self.ny_new-1))/np.sinh(self.b)
        return y
    ...
    def main(self):
        ...
        for var_no, var in enumerate(variables):
            ...
            zero_indices = np.where(z_extend == 0)

# Use these indices to update z_extend with corresponding values from full_grid_interpolated
            z_extend[zero_indices] = z_extend_full[zero_indices]
