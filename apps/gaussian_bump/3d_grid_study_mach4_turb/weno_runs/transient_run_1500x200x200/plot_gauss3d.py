...
print('exiting after wall u plots')
exit()
# ###################################################################################################
# #                                                                                                 #
# #                y-z plane slices                                                                 #
...
for idx in x_idx:
    ...
    if idx==50:
        U=ax1.contourf(z[:,:,x_loc], y[:,:,x_loc], w[:,:,x_loc], levels=list(np.linspace(np.min(w[:,:,idx]), np.max(w[:,:,idx]), 30)), cmap=cm.jet)
    else:
        U=ax1.contourf(z[:,:,x_loc], y[:,:,x_loc], u[:,:,x_loc], levels=list(np.linspace(np.min(u[:,:,:]), np.max(u[:,:,:]), 30)), cmap=cm.jet)
...
fig3, axarr = plt.subplots(5, 2, figsize=(6,4.3))
...
for j in range(len(x_idx)):
    for i, idx in enumerate(x_idx[j]):
        ...
        axarr[i,j].set_title('x=%d'%idx,fontsize=5)
...
cbar_ax = fig3.add_axes([0.2,0.01,0.6,0.02])
