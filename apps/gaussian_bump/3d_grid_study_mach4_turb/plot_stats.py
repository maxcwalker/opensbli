...
try:
    ...
    fname='stats_output.h5'
directory = './stats_plots/'
...
rdum=read_dataset(f, 'rhomean_B0')
rudum=read_dataset(f, 'rhou0mean_B0')
rvdum=read_dataset(f, 'rhou1mean_B0')
rwdum=read_dataset(f, 'rhou2mean_B0')
rEdum=read_dataset(f, 'E_mean_B0')

# print('Setting flowfield arrays')
vals = 2
x=x0dum[vals:-vals,vals:-vals,vals:-vals]
y=x1dum[vals:-vals,vals:-vals,vals:-vals]
z=x2dum[vals:-vals,vals:-vals,vals:-vals]
rho=rdum[vals:-vals,vals:-vals,vals:-vals]
rhou=rudum[vals:-vals,vals:-vals,vals:-vals]
rhov=rvdum[vals:-vals,vals:-vals,vals:-vals]
rhow=rwdum[vals:-vals,vals:-vals,vals:-vals]
rhoE=rEdum[vals:-vals,vals:-vals,vals:-vals]
D11 = read_dataset(g, "D11_B0")
D11=D11[vals:-vals,vals:-vals,vals:-vals]
...
# ax2.plot(Rex, St_rey_ana,label='Reynolds Analogy \nStanton Number')
ax2.plot(Rex, St_acc,color='k',label='Stanton Number\nEquation')
ax2.set_xlabel(r'$Re_x$', fontsize=20)
ax2.set_ylabel(r'$St$', fontsize=20)
# ax2.set_ylim([-0.2,3])
ax2.set_xlim([min(Rex),max(Rex)])
# ax2.legend(loc='best')
# ax2.set_title('Reynolds number vs Stanton number', fontsize=15)
fig2.savefig(directory+'Rex_St.pdf', bbox_inches='tight')
...
T = spanaverage(T)
xp0 = int(len(x[0,0,:])/Lx *50)
xp1 = int(len(x[0,0,:])/Lx *170)
xp2 = int(len(x[0,0,:])/Lx *350)


#Temperature
fig3, ax3 = plt.subplots(1)
ax3.plot(T[:,xp0],y[0,:,xp0], label='x=50',color="k",linewidth=1)
ax3.plot(T[:,xp1],y[0,:,xp1], label='x=170',color="b",linewidth=1)
ax3.plot(T[:,xp2],y[0,:,xp2], label='x=350',color="g",linewidth=1)
ax3.set_ylim([0,10])
ax3.legend()
ax3.grid()
ax3.set_ylabel(r'$y$',fontsize=20); ax3.set_xlabel(r"$T mean$",fontsize=20)
fig3.savefig(directory + "temp_profiles.pdf",bbox_inches='tight')

#u0 vel
fig4, ax4 = plt.subplots(1)
ax4.plot(u_avg[:,xp0],y[0,:,xp0], label='x=50',color="k",linewidth=1)
ax4.plot(u_avg[:,xp1],y[0,:,xp1], label='x=170',color="b",linewidth=1)
ax4.plot(u_avg[:,xp2],y[0,:,xp2], label='x=350',color="g",linewidth=1)
ax4.set_ylim([0,8])
ax4.legend()
ax4.grid()
ax4.set_ylabel(r'$y$',fontsize=20); ax4.set_xlabel(r"$u0 mean$",fontsize=20)
fig4.savefig(directory + "uvel_profiles.pdf",bbox_inches='tight')
