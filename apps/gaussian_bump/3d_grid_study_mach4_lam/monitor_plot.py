...
for i, val in enumerate(filtered_monitors):
    range1=0
    range2=29500
    ax[i].plot(df[val][range1:range2], (df['Time'][range1:range2]), color='k', linewidth=0.7)  # Swap x and y, reverse time 29400:32000
    ax[i].set_xlabel(val[2:], loc='center', labelpad=20, fontsize=15)  # Set monitor label on x-axis at the top
    ax[i].grid(True)
    ...
    ax[i].set_ylim([df['Time'][range1],df['Time'][range2]])
...
fig.text(0.5, -0.05, 'Streamwise Location', ha='center', va='center', fontsize=20)
