...
def reorder_and_generate_coordinates_incremental(filename, y_max, num_y_points, a, output_file):
    ...
    # Print grid and domain dimensions
    print(f"x-length: {unique_x[-1] - unique_x[0]}")
    print(f"z-length: {unique_z[-1] - unique_z[0]}")
    print(f"nx: {num_x_points}, ny: {num_y_points}, nz: {num_z_points}")
    ...
    with open(output_file, 'w') as f:
        ...
        for z_val in unique_z:
            print(f"{z_val:.2f} out of {unique_z[-1]:.2f}")
...
if __name__ == "__main__":
    filename = 'output_interpolated_extended.dat'  # Replace with your output file name
    output_file = 'new_output.dat'  # File to save the new coordinates
    y_max = 100.0  # Maximum y coordinate (farfield) LX1
    num_y_points = 300  # Number of points in the y direction
    ...
    if coordinates.size == 0:
        ...
        y = coordinates[:, 1]
        z = coordinates[:, 2]
        print(f"x-length: {x.max() - x.min():.2f}")
        print(f"y-length: {y_max:.2f}")
        print(f"z-length: {z.max() - z.min():.2f}")
        print(f"Grid dimensions - nx: {len(np.unique(x))}, ny: {num_y_points}, nz: {len(np.unique(z))}")
