...
class plots_3D(plotFunctions):
    ...
    def plot_3d(self, x, y, z, data, label):
        ...
        # Plotting the data using contour
        X, Y = np.meshgrid(x.flatten(), y.flatten())
        contour = ax.contourf(X, Y, data.flatten(), cmap='jet')

        fig.colorbar(contour)
        plt.title(f'3D Contour Visualization of {label}')
        fig.savefig(f'3D_contour_{label}.pdf')
        plt.show()
...
class slice_plots(plotFunctions):
    ...
    def plot_slice(self, x, y, data, plane, idx, label):
        ...
        contour = ax.contourf(x, y, data, levels=25, cmap='jet')

        # Adjust the color bar to match the height of the plot
        ...
        ax.set_ylabel(f'{plane[2]} axis')

        # # Only create an inset for the z-y slice at index 750
        # if plane == "z-y" and idx == 750:
        #     # Create an inset axis for the scatter plot
        #     inset_ax = inset_axes(ax, width="40%", height="40%", loc='upper left')  # You can change loc as needed

        #     # Define limits for the inset axis (adjust these based on your data)
        #     x1, x2 = 47.5, 52.5
        #     y1, y2 = 21, 26
        #     inset_ax.set_xlim(x1, x2)  # Example limits for zooming in
        #     inset_ax.set_ylim(y1, y2)  # Example limits for zooming in
        #     inset_ax.set_aspect('equal')
        #     # Contour plot for the inset axis
        #     contour_inset = inset_ax.contourf(x, y, data, cmap='jet')
        ...
        #     zoom_rect = patches.Rectangle((x1, y1), 5, 5, linewidth=1, edgecolor='black', facecolor='none', linestyle='--')  # Adjust dimensions if needed
    ...
    def plot_slices(self, x, y, z, data, label):
        ...
        x_slices = [10, 20, 30, 50, 100, 200, 500, 750, 1000, 1300]  # z-y plane
        y_slices = [1, 5, 15, 25]  # x-z plane
        z_slices = [3, 13, 23, int(len(x[:, 0, 0]) / 2)]  # x-y plane
