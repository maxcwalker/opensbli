...
//
__constant__ int dims_monitor_5_rhou0_B0 [2][1];
static int dims_monitor_5_rhou0_B0_h [2][1] = {{0}};

//user function
__device__

void monitor_5_rhou0_B0_gpu(const ACC<double> &rhou0_B0,
  double *reduce_5_rhou0_B0){
*reduce_5_rhou0_B0 = rhou0_B0(0,0);
}



__global__ void ops_monitor_5_rhou0_B0(
...
int size1 ){
  ...
  if (size0==-1) dims_monitor_5_rhou0_B0[0][0]=0;
  ...
  arg0 += idx_x * 1*1 + idx_y * 1*1 * dims_monitor_5_rhou0_B0[0][0];

  if (idx_x < size0 && idx_y < size1) {
    const ACC<double> argp0(dims_monitor_5_rhou0_B0[0][0], arg0);
    monitor_5_rhou0_B0_gpu(argp0, arg1_l);
  }
  for (int d=0; d<1; d++)
...
#else
void ops_par_loop_monitor_5_rhou0_B0_execute(ops_kernel_descriptor *desc) {
  int dim = desc->dim;
  #if OPS_MPI
  ...
  if (xdim0 != dims_monitor_5_rhou0_B0_h[0][0]) {
    dims_monitor_5_rhou0_B0_h[0][0] = xdim0;
    hipSafeCall(block->instance->ostream(), hipMemcpyToSymbol(HIP_SYMBOL(dims_monitor_5_rhou0_B0), dims_monitor_5_rhou0_B0_h, sizeof(dims_monitor_5_rhou0_B0)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    hipLaunchKernelGGL(ops_monitor_5_rhou0_B0,grid ,tblock ,nshared ,0 , (double *)p_a[0], (double *)arg1.data_d,x_size, y_size);
...
void ops_par_loop_monitor_5_rhou0_B0(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  desc->args[1] = arg1;
  desc->function = ops_par_loop_monitor_5_rhou0_B0_execute;
  if (block->instance->OPS_diags > 1) {
    ops_timing_realloc(block->instance,58,"monitor_5_rhou0_B0");
  }
