...
//
__constant__ int dims_opensbliblock00Kernel067 [3][2];
static int dims_opensbliblock00Kernel067_h [3][2] = {{0}};

//user function
__device__

void opensbliblock00Kernel067_gpu(const ACC<double> &T_B0,
  ACC<double> &wk7_B0,
  const int *idx)
{
   if (idx[1] == 0){
...
void ops_par_loop_opensbliblock00Kernel067(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2) {
  ...
  create_kerneldesc_and_enque(name, args, 3, 32, dim, 1, range, block, ops_par_loop_opensbliblock00Kernel067_execute);
}
#endif
