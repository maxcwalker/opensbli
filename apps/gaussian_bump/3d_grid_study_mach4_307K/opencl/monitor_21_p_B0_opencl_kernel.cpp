// Auto-generated at 2024-10-10 09:28:39.042659 by ops-translator legacy
    ...
    static bool isbuilt_monitor_21_p_B0 = false;

    void buildOpenCLKernels_monitor_21_p_B0(OPS_instance *instance, int xdim0, int ydim0) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_monitor_21_p_B0) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./opencl/monitor_21_p_B0.cl"};
        ...
        instance->ostream() <<"Compiling monitor_21_p_B0 "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_monitor_21_p_B0=%d  -Dydim0_monitor_21_p_B0=%d ", pPath, 32,xdim0,ydim0);
            else
              sprintf(buildOpts,"-cl-mad-enable -I%s/include -DOPS_WARPSIZE=%d  -Dxdim0_monitor_21_p_B0=%d  -Dydim0_monitor_21_p_B0=%d ", pPath, 32,xdim0,ydim0);
          else {
          ...
          instance->ostream() << "compiling monitor_21_p_B0 -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[61] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_monitor_21_p_B0", &ret);
        clSafeCall( ret );

        isbuilt_monitor_21_p_B0 = true;
        free(source_str[0]);
      }
...
// host stub function
void ops_par_loop_monitor_21_p_B0(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  #ifdef CHECKPOINTING
  if (!ops_checkpointing_before(args,2,range,61)) return;
  #endif

  if (block->instance->OPS_diags > 1) {
      ops_timing_realloc(block->instance,61,"monitor_21_p_B0");
      block->instance->OPS_kernels[61].count++;
  ...
  buildOpenCLKernels_monitor_21_p_B0(block->instance,
  xdim0,ydim0);
  ...
  if (block->instance->OPS_diags > 1) {
      ...
      block->instance->OPS_kernels[61].mpi_time += t2-t1;
  }
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
      ...
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 0, sizeof(cl_mem), (void*) &arg0.data_d ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 1, sizeof(cl_mem), (void*) &arg1.data_d ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 2, nthread*sizeof(double), NULL));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 3, sizeof(cl_int), (void*) &r_bytes1 ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 4, sizeof(cl_int), (void*) &base0 ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 5, sizeof(cl_int), (void*) &x_size ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 6, sizeof(cl_int), (void*) &y_size ));
      clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[61], 7, sizeof(cl_int), (void*) &z_size ));

      //call/enqueue opencl kernel wrapper function
      clSafeCall( clEnqueueNDRangeKernel(block->instance->opencl_instance->OPS_opencl_core.command_queue, block->instance->opencl_instance->OPS_opencl_core.kernel[61], 3, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL) );
  }
  if (block->instance->OPS_diags>1) {
  ...
  if (block->instance->OPS_diags > 1) {
      ops_timers_core(&c1,&t1);
      block->instance->OPS_kernels[61].time += t1-t2;
  }
  ...
  if (block->instance->OPS_diags > 1) {
      ...
      ops_timers_core(&c2,&t2);
      block->instance->OPS_kernels[61].mpi_time += t2-t1;
      block->instance->OPS_kernels[61].transfer += ops_compute_transfer(dim, start, end, &arg0);
  }
}
