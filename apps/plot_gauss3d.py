...
for idx in y_idx:
    ...
    # print('the index for location 100 is: %d' % list(x[0,idx,:]).index(100))
    xp1,xp2 = int(len(x[0,idx,:])/Lx*100), int(len(x[0,idx,:])/Lx*200)  # This line only works if no stretching in x!!!!
    print(xp1,xp2)
    try:
    ...
    ax1.set_title(r'Planview contours at y=%d'%idx,fontsize=10)
