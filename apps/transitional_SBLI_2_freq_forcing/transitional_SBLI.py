...
#body_force = Eq(DataObject('BF'), A*exp(-((x0-xF)**2+(x1-yF)**2))*cos(bta*x2)*sin(omega1*dt*current_iter) + A*exp(-((x0-xF)**2+(x1-yF)**2))*cos(bta*x2)*sin(omega2*dt*current_iter) )
...
body_force = Eq(DataObject('BF'), 0.5*A*exp(-((x0-xF)**2+(x1-yF)**2))*( cos(bta*x2-omega1*dt*current_iter) + cos(bta*x2+omega1*dt*current_iter) + cos(bta*x2-omega2*dt*current_iter) + cos(bta*x2+omega2*dt*current_iter) ) )
...
arrays = ['rho', 'rho', 'rho', 'rho', 'rho']
arrays = [block.location_dataset('%s' % dset) for dset in arrays]
indices = [(110, 100, 100), (800, 0, 100), (1100, 0, 100), (1600, 0, 100), ('block0np0-50', 0, 100)]
SM = SimulationMonitor(arrays, indices, block, print_frequency=16, fp_precision=12, output_file='output.log')
...
constants = ['gama', 'Minf', 'Pr', 'Re', 'Twall', 'dt', 'niter', 'block0np0', 'block0np1', 'block0np2',
             'Delta0block0', 'Delta1block0', 'Delta2block0', 'SuthT', 'RefT', 'eps', 'Lx1', 'by', 'A', 'bta', 'omega1', 'omega2', 'xF', 'yF', 'teno_a1', 'teno_a2', 'epsilon', 'restart_iteration_no']
values = ['1.4', '1.5', '0.72', '750.0', '1.3809973268575328', '0.01', '525000', '2050', '325', '200',
          '375.0/(block0np0-1)', '140.0/(block0np1-1)', '27.32/(block0np2)', '110.4', '202.17', '1e-30', '140.0', '5.0', '2.5e-3', '0.23', '0.100', '0.104', '20.0', '4.0', '9.5', '3.5', '1.0e-16', '0']
