...
void ops_par_loop_opensbliblock00Kernel009_execute(ops_kernel_descriptor *desc) {
  ...
  double * __restrict__ phi_B0_p = (double *)(args[2].data_d + base2);
  ...
  int consts_bytes = 0;
  consts_bytes += ROUND_UP(args[3].dim*sizeof(double));

  reallocConstArrays(block->instance,consts_bytes);
  consts_bytes = 0;

  args[3].data = block->instance->OPS_consts_h + consts_bytes;
  args[3].data_d = block->instance->OPS_consts_d + consts_bytes;
  for ( int d=0; d<args[3].dim; d++ ){
    ((double *)args[3].data)[d] = arg3h[d];
  }
  consts_bytes += ROUND_UP(args[3].dim*sizeof(double));

  mvConstArraysToDevice(block->instance,consts_bytes);

  double * __restrict__ rknew = (double *)args[3].data_d;
...
void ops_par_loop_opensbliblock00Kernel009(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1, ops_arg arg2, ops_arg arg3) {
  ...
  create_kerneldesc_and_enque(name, args, 4, 4, dim, 0, range, block, ops_par_loop_opensbliblock00Kernel009_execute);
}
#endif
